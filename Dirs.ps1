...
{
     #Récuperer le chemin à partir de la textbox et afficher les fichiers et dossiers contenus dans le répertoire
...
{
    #Récuperer le contenu des textbox et le stocker dans des variables
    $Path     = $TextBoxPath.Text
    $Group    = $TextBoxGroup.Text
    $Dir      = $TextBoxDir.Text
    
    #Vérifier si aucun des textbox ne sont vide
    if(($Path.Length -ne 0) -or ($Group.Length -ne 0) -or ($Dir.Length -ne 0))
    { 
        #Si oui alors vérifier que le groupe existe sinon le créer
        ...
        #Si le nom du groupe contient « READ » alors définir les droits d’accès en lecture seulement, sinon 
        #mettre en écriture
        if($Group -Match "READ")
        {
            $AccessRule = New-Object System.Security.AccessControl.FileSystemAccessRule `
            @("sacredlubin\$Group", "ReadAndExecute", "None" , "none", "Allow")
        }
        else
        {
            $AccessRule = New-Object System.Security.AccessControl.FileSystemAccessRule `
            @("sacredlubin\$Group", "Modify", "containerinherit", "none", "Allow")
        }
        ...
        #Appliquer les droits NTFS sur le dossier
        $Path = "$Path\$Dir"
        $Acl = Get-Acl $Path     
        $AccessRule
        $Acl.AddAccessRule($accessRule)
        $Acl | Set-Acl $Path
        #Afficher une popup « Succès »
