...
$ButtonRDP.Add_Click(
{
    ...
    #Si il y a minimum une session non active et non service mal déconnectée alors
    if ($Session)
    {
    ...
    $Session = qwinsta /server:$Server2 | select-string -notmatch "Actif" | select-string -notmatch "services"
    if ($Session)
    {
        $Session| % { 
